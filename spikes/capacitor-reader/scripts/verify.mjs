...
const spikeDir = path.resolve(scriptsDir, '..');
const repoRoot = path.resolve(spikeDir, '..', '..');

const preloadPath = path.join(repoRoot, 'beytelhikma-electron', 'src', 'preload', 'preload.cjs');
const vraiRepoPath = path.join(repoRoot, 'beytelhikma-electron', 'src', 'renderer', 'js', 'repository.js');
const specPath = path.join(repoRoot, 'docs', 'superpowers', 'specs', '2026-08-02-capacitor-lecteur-design.md');
const shimPath = path.join(spikeDir, 'src', 'repository.capacitor.js');
const wwwRelatif = 'spikes/capacitor-reader/www';
