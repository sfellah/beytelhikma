#!/usr/bin/env node
/**
 * Régénère `www/` depuis `beytelhikma-electron/src/renderer/`.
 *
...
const rendererDir = path.join(repoRoot, 'beytelhikma-electron', 'src', 'renderer');
const sharedDir = path.join(repoRoot, 'beytelhikma-electron', 'src', 'shared');
const wwwDir = path.join(spikeDir, 'www');
