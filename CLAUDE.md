...
**Deux modes de lecture** (`reader.mode`, persisté) : `page`, une page imprimée par écran, et `scroll`, un fil continu. Une annotation s'ancre sur la page qui porte la sélection, pas sur la page « courante » : en fil continu, ce n'est pas la même.

**Le fil monte le livre entier, et ne démonte rien.** Il n'y a plus de tranche glissante — `FLOW_KEEP` et `#trim` ont disparu : on descend, on descend, et tout est là, y compris derrière soi. Mais il ne l'attend pas. On ouvre sur quelques pages autour de la reprise (`#fill`), puis `#startBackfill` monte le reste **dans les creux**, par tranches de `BACKFILL_STEP`, **dans les deux sens sans court-circuit** — le lecteur ouvre rarement à la page 1, ce qui précède la reprise compte autant que ce qui la suit. Un `await` du livre complet avant le premier dessin serait un refus déguisé.

Le remplissage passe par `#extendEnd` / `#extendStart` et **n'invente pas de second chemin** : la compensation de `scrollTop` — ce qui empêche la lecture de sauter quand du texte s'ajoute au-dessus d'elle — n'est écrite qu'à un endroit. Il cède la place au geste (`#extending` déjà pris ⇒ on repasse plus tard) et meurt trois fois : `dispose`, passage en mode `page`, nouveau départ. Le jeton compte pour deux — il annule le tour *programmé* et périme le tour *en vol*, un tour étant fait d'attentes.

Porter le livre entier interdit tout ce qui serait proportionnel au nombre de pages montées sur un chemin qui se rejoue. Cinq passages ont dû changer, chacun tenu par un test :

- `#visibleBlock` — dichotomie, plus balayage : un `getBoundingClientRect()` par page, à **chaque** évènement de défilement, force autant de dispositions complètes.
- `#setCurrent` — `is-current` posé sur les deux blocs concernés, et `#syncBookmarkButton` seul : franchir une page ne change le signet d'aucune autre.
- `#syncChapters(from, to)` — par tranche ajoutée. Relire tout le fil à chaque tranche coûtait le carré du livre.
- `#chapterFor` — dichotomie sur le sommaire, qui est trié par `pageId` (le balayage le supposait déjà, il s'arrêtait au premier dépassement). Mesuré sur le corpus, `pages × sommaire` vaut 54 millions de tours au 99ᵉ centile et 17 milliards sur le plus gros livre.
- `#afterAnnotationChange(pageId)` — seule la page touchée se repeint. Poser une teinte sur trois mots relançait `renderBookHtml` sur tout le fil.

`#extendStart` pose sa tranche **d'un coup**, entre une seule paire de mesures : chaque `scrollHeight` lu force une disposition, et les lire page par page rendait le remplissage quadratique. D'où `BACKFILL_STEP` (12) plus large que `FLOW_STEP` (3) — quatre fois moins de tranches ajoutées en tête, donc quatre fois moins de dispositions forcées.

**Ce que ça coûte, mesuré.** Un livre réel du corpus (`sh-7745`, 230 pages) rend 13 nœuds de corps par page ; avec le chrome du bloc (section, signet, chapitre, article, notes) et la densité médiane du corpus, compter **~25 nœuds DOM par page**. Le corpus donne : médiane 206 pages (~5 000 nœuds), p90 1 966 (~49 000), p95 3 876 (~97 000), p99 10 766 (~270 000), et un maximum à **124 569 pages, soit ~3,1 millions de nœuds**. 30 % des livres dépassent 500 pages, 1,2 % dépassent 10 000. Le seuil au-delà duquel ce sera tenu pour intenable n'est pas fixé : il n'y a **aucun garde-fou** aujourd'hui, c'est un choix assumé, et le symptôme attendu sur les plus gros livres est un remplissage qui n'en finit pas puis une mémoire qui monte de plusieurs centaines de mégaoctets. Le garde-fou proposé, s'il faut en poser un : ne remplir intégralement qu'au-dessous de ~3 000 pages (93 % du corpus) et garder la tranche glissante au-delà.

**Le fil est un seul bloc de texte.** Il ne porte plus aucune marque de page : ni pied imprimé, ni numéro entre deux pages, ni filet, ni marge. Le dernier paragraphe de la page N et le premier de la N+1 se suivent avec l'écart d'un paragraphe, celui que `.reader__page p` donne déjà. La découpe n'a jamais été une donnée du texte — c'est celle du fichier, et l'afficher faisait compter les pages au lieu de lire. **Le mode `page` ne change en rien** : une feuille par écran, son pied imprimé, son feuilletage.

Un seul bloc **à l'œil**, pas dans l'arbre : `page_id` porte toujours l'ancrage des annotations, `saveProgress` écrit toujours un `sequenceNum`, le fenêtrage monte toujours des `.reader__block`, `?page=` mène toujours quelque part. Le modèle de données et l'application Android en dépendent.

Deux gestes, deux moyens, et la raison de chacun :

- **Le pied imprimé n'est pas dessiné** au fil (`#makeBlock`), plutôt que dessiné puis masqué. Caché, il resterait dans l'arbre, dans la sélection et dans le texte copié — au milieu d'une phrase que le fil vient de recoudre. Les blocs sont entièrement reconstruits à chaque bascule de mode, ne pas le monter ne coûte donc rien au retour.
- **Le ruban perd ses chevrons et sa fraction par une règle CSS** (`.reader--scroll .reader__pager`), pas par un `if` : ces trois nœuds ont un sens en mode page, et la touche `V` bascule le mode sans reconstruire la barre. Ils restent tenus à jour par `#setCurrent`, donc justes à l'instant où ils reviennent. Ce qui reste au fil, c'est ce qui dit *où l'on en est* : la jauge et le pourcentage. Sans page à l'écran, « ١٢ / ٢٣٠ » et les deux chevrons ne désignent plus rien de visible.

Reste une coupure, et une seule : **le titre de chapitre, quand il change**. Il se répétait à chaque page ; au fil il reviendrait toutes les vingt lignes et redécouperait en pages le texte qu'on vient de recoudre. `#syncChapters` compare avec le bloc **réellement monté** juste avant — pas avec `#pages`, le cache, qui garde des pages que le fil n'a pas encore reprises, et pas avec une variable de parcours : le fil s'étend par les deux bouts, et la page qui l'ouvrait doit se taire dès qu'une voisine la précède. D'où l'appel après chaque tranche ajoutée, en tête comme en queue.
